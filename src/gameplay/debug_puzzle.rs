...
fn dirs(e: bool, n: bool, w: bool, s: bool) -> DirectionSet {
    ...
    if e { set.insert(Direction::E); }
    if n { set.insert(Direction::N); }
    if w { set.insert(Direction::W); }
    if s { set.insert(Direction::S); }
    set
}
...
fn ldirs(e: usize, n: usize, w: usize, s: usize) -> DirectionMap<LayerConnection> {
    ...
    if e > 0 { map[Direction::E] = if e == 1 { LayerConnection::Layer0 } else { LayerConnection::Layer1 }; }
    if n > 0 { map[Direction::N] = if n == 1 { LayerConnection::Layer0 } else { LayerConnection::Layer1 }; }
    if w > 0 { map[Direction::W] = if w == 1 { LayerConnection::Layer0 } else { LayerConnection::Layer1 }; }
    if s > 0 { map[Direction::S] = if s == 1 { LayerConnection::Layer0 } else { LayerConnection::Layer1 }; }
    map
}
