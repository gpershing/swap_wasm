...
use crate::{
    gameplay::{Color, LayerConnection, PuzzleCell},
    grids::{Direction, DirectionMap, DirectionSet, Grid, GridIndex},
};

use super::{
    solutions::{GeneratorCell, GeneratorFailure},
    GeneratorSettings,
};
...
fn get_all_indices(
    grid: &ConnectionGrid,
    match_color: Color,
) -> impl Iterator<Item = CxnGridIndex> + '_ {
    grid.iter().flat_map(move |(grid_index, cell)| match cell {
        ConnectionCell::Single(_, color) if *color == match_color => vec![CxnGridIndex {
            grid_index,
            layer_index: None,
        }],
        ConnectionCell::Single(_, _) => vec![],
        ConnectionCell::Intersection(_, colors) => colors
            .iter()
            .enumerate()
            .filter_map(|(i, color)| {
                (*color == match_color).then_some(CxnGridIndex {
                    grid_index,
                    layer_index: Some(i),
                })
            })
...
pub fn connect_groups(
    ...
    for (pos, cell) in gen_grid {
        grid.insert(
            pos,
            match cell {
                GeneratorCell::SingleGroup(color) => {
                    ConnectionCell::Single(DirectionSet::empty(), color)
                }
                GeneratorCell::Intersection(color_a, color_b) => {
                    intersections_left -= 1;
                    ConnectionCell::Intersection(
                        DirectionMap::new_with_repeat(LayerConnection::None),
                        [color_a, color_b],
                    )
                }
            },
    ...
    for color in Color::ALL {
        ...
        for index in indices.iter() {
            let neighbors: Vec<_> = grid
                .iter_neighbors(index.grid_index)
                .map(|n| (n.0, n.1))
                .collect();
            for neighbor in neighbors {
                for layer_index in [None, Some(0), Some(1)] {
                    let neighbor_index = CxnGridIndex {
                        grid_index: neighbor.0,
                        layer_index,
                    };
    ...
    for color in present_colors.iter() {
        ...
        while !potential_sources.is_empty() && chance(generator_settings.extra_source_chance) {
            all_sources.insert(
                potential_sources
                    .swap_remove(random_index(potential_sources.len()))
                    .grid_index,
                *color,
            );
        }
...
fn knockout_loops(
    grid: &mut ConnectionGrid,
    color: Color,
    intersections_left: &mut usize,
    generator_settings: &GeneratorSettings,
) {
    loop {
        ...
        while let Some(exploring) = to_explore.pop_front() {
            if !not_checked.remove(&exploring) {
                continue;
            }
            ...
            for neighbor in grid.iter_neighbors_for(exploring.grid_index, layer.iter_set()) {
                ...
                for layer_index in [None, Some(0), Some(1)] {
                    if layer_index.is_some() != is_intersection {
                        continue;
                    }
                    let neighbor_index = CxnGridIndex {
                        grid_index: neighbor.0,
                    ...
                    if not_checked.contains(&neighbor_index)
                        && get_layer(grid, neighbor_index).contains(neighbor.1.inverse())
                    {
        ...
        if let Some((break_to, (break_from, direction))) = counts
            .iter()
            .filter(|(_, count)| **count > 1)
            .filter_map(|(p, _)| prevs.get(p).map(|prev| (*p, *prev)))
            .choose(&mut rand::thread_rng())
        {
            if break_to.layer_index.is_none()
                ...
                && chance(generator_settings.intersection_chance)
            {
                ...
                *grid.get_mut(break_to.grid_index).unwrap() =
                    ConnectionCell::Intersection(connections, [color, color])
            } else {
