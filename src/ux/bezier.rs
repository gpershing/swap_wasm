...
pub fn generate_lut(curve: [Pos2; 4], steps: usize) -> Vec<Pos2> {
    ...
    (0..=steps)
        .map(|step| compute(curve, (step as f32) * s_inv))
        .collect()
}
...
impl CubicBezierMesh {
    pub fn new(curve: &[CubicBezierPoint], width: f32, feathering: f32, precision: f32) -> Self {
        ...
        for idx in 1..curve.len() {
            beziers.push([
                curve[idx - 1].point,
                curve[idx - 1].point + curve[idx - 1].handle_out(),
                curve[idx].point + curve[idx].handle_in(),
                curve[idx].point,
            ]);
        }
        ...
        vertices.push(CubicBezierMeshVertex {
            position: previous_vertex + previous_normal * feather_width,
            t: 0.0,
        });
        vertices.push(CubicBezierMeshVertex {
            position: previous_vertex + previous_normal * half_width,
            t: 0.0,
        });
        vertices.push(CubicBezierMeshVertex {
            position: previous_vertex - previous_normal * half_width,
        ...
        for (index, curve) in beziers.iter().enumerate() {
            ...
            for (lut_index, vertex) in lut.iter().enumerate() {
                ...
                vertices.push(CubicBezierMeshVertex {
                    position: *vertex + normal * feather_width,
                    t,
                });
                vertices.push(CubicBezierMeshVertex {
                    position: *vertex + normal * half_width,
                    t,
                });
                vertices.push(CubicBezierMeshVertex {
                    position: *vertex - normal * half_width,
