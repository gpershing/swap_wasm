...
fn draw_swap_indicator_with_color(
    painter: &Painter,
    center: Pos2,
    t: f32,
    color: Color32,
    data: &SwapsLeftDrawData<'_>,
) {
    let closed = false;
    ...
    painter.extend(
        (0..max)
            .map(|i| (i as f32 * (1.0 - t)) * TAU / 6.0 + TAU / 4.0)
            .flat_map(|theta| {
...
fn draw_swap_indicator(painter: &Painter, center: Pos2, t: f32, data: &SwapsLeftDrawData<'_>) {
    ...
    draw_swap_indicator_with_color(
        painter,
        center,
        t,
        data.palette.get(Color::Purple).gamma_multiply(1.0 - t),
        data,
    );
}
...
impl SwapsLeftAnimation {
    ...
    pub fn draw(
        ...
        for i in 0..swap_limit {
            ...
            let center = Pos2::new(
                data.rect.right() * center_t + data.rect.left() * (1.0 - center_t),
                data.rect.center().y,
            );
