use egui::{
    ...
    Button, EventFilter, Modifiers, Painter, Pos2, Rect, Response, Sense, Ui, Vec2,
};

use crate::{
    gameplay::{Color, PlayingPuzzle, PuzzleSolveState, SwapRecord},
    grids::GridIndex,
};

use super::{
    background::{BackgroundAnimation, BackgroundAnimationDrawData},
    cell::{draw_cell, CellDrawData},
    palette::{self, Palette},
    simulation::Simulation,
    swaps_left::{SwapsLeftAnimation, SwapsLeftDrawData},
    SegmentMeshData,
};
    ...
    fn get_grid_pos(
        ...
        ui.ctx().pointer_interact_pos().and_then(|pos| {
            ...
            if game_coord_round.x < 0.0 || game_coord_round.y < 0.0 {
                ...
                let game_coord =
                    GridIndex::new(game_coord_round.x as usize, game_coord_round.y as usize);
...
fn control_button(
    ...
    ui.vertical_centered_justified(move |ui| ui.add_enabled(!disabled, button))
        .inner
        .clicked()
}
...
fn handle_controls(
    ...
    if controls.undo {
        if let Some(record) = puzzle.try_undo() {
            state.simulation.swap(SwapRecord {
                a: record.a,
                b: record.b,
                a_rotation: record.b_rotation.inverse(),
                b_rotation: record.a_rotation.inverse(),
            });
...
pub fn update_game(
    ...
    let cell_size = (game_width_max / bounds.width as f32)
        .min(game_height_max / bounds.height as f32)
        .min(style.scale)
        .floor()
        .max(MAX_SIZE);
    ...
    let controls_size = Vec2::new(
        (game_size.x - ui.spacing().item_spacing.x)
            .max(3.0 * cell_size)
            .min(max_size.x - margin.sum().x),
        CONTROLS_HEIGHT,
    );
    let controls_rect = Rect::from_center_size(
        Pos2 {
            x: center_x,
            y: painter.clip_rect().top() + controls_size.y * 0.5,
        },
        controls_size,
    );
    let game_rect = Rect::from_center_size(
        Pos2 {
            x: center_x,
            y: controls_rect.bottom() + game_size.y * 0.5,
        },
        game_size,
    );
    let indicators_size = Vec2::new(game_size.x, INDICATORS_HEIGHT);
    let indicators_rect = Rect::from_center_size(
        Pos2 {
            x: center_x,
            y: game_rect.bottom() + indicators_size.y * 0.5,
        },
        indicators_size,
    );

    update_game_after_sizing(
        ui,
        puzzle,
        state,
        puzzle_state,
...
fn update_game_after_sizing(
    ...
    let game_coords = Rect::from_min_size(
        Pos2::new(-0.5, -0.5),
        Vec2 {
    ...
    let mut controls_response = ui
        .allocate_ui_at_rect(controls_rect, |ui| {
            draw_controls(
                ui,
                palette,
                puzzle_state.hint_shown || puzzle.swaps_made() > 0,
                puzzle_state.solved,
            )
        })
        .inner;
    ...
    let swap_action = match update_input(
        &mut state.input.input,
        ui,
        response,
        puzzle,
        &to_game_coords,
    ...
    state.swaps_left_animation.draw(
        &painter,
        puzzle.swaps_made(),
        puzzle.swap_limit(),
        dt,
    ...
    for (grid_pos, cell) in puzzle.iter_cells() {
        ...
        let show_hint = puzzle_state.hint_shown
            && puzzle.puzzle().hint() == grid_pos
            && puzzle.swaps_made() == 0;
        state.backgound_animation.draw_background_cell(
            &painter,
            palette,
            cell,
            dt,
            BackgroundAnimationDrawData {
                index: grid_pos,
                center,
                scale: cell_size,
                show_hint,
    ...
    for (grid_pos, cell) in puzzle.iter_cells() {
        ...
        let size =
            if Some(grid_pos) == state.input.selected || Some(grid_pos) == state.input.highlight {
                cell_size * 0.85
            } else {
                cell_size
            };
