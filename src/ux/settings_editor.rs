...
pub fn edit_generator_settings(
    ctx: &Context,
    use_settings: &mut bool,
...
fn edit_advanced_settings_grid(ui: &mut Ui, settings: &mut GeneratorSettings) {
    ui.label("Missing chance")
        .on_hover_text("Chance that a cell in the grid will have nothing.");
    ui.add(Slider::new(&mut settings.missing_chance, 0.0..=1.0));
    ui.end_row();

    ui.label("Maximum missing")
        .on_hover_text("Maximum number of missing tiles.");
    ui.add(Slider::new(&mut settings.missing, 0..=4));
    ui.end_row();

    ui.label("Extra region chance")
        .on_hover_text("Chance that additional regions will be added beyond the minimum.");
    ui.add(Slider::new(&mut settings.extra_region_chance, 0.0..=1.0));
    ui.end_row();

    ui.label("Extra source chance")
        .on_hover_text("Chance that a region will have more than one source tile.");
    ui.add(Slider::new(&mut settings.extra_source_chance, 0.0..=1.0));
    ui.end_row();

    ui.label("Intersection chance")
        .on_hover_text("Chance that any attempt to make an intersection tile will succeed.");
    ui.add(Slider::new(&mut settings.intersection_chance, 0.0..=1.0));
    ui.end_row();

    ui.label("Maximum intersections")
        .on_hover_text("Maximum number of intersection tiles.");
    ui.add(Slider::new(&mut settings.max_intersections, 0..=5));
    ui.end_row();

    ui.label("Knockout loop chance")
        .on_hover_text("Chance that any loop will be removed from a region.");
    ui.add(Slider::new(&mut settings.knockout_loop_chance, 0.0..=1.0));
    ui.end_row();

    ui.label("Check solution depth")
        .on_hover_text("How deep to explore when looking for a possible shorter solution.");
    ui.add(Slider::new(
        &mut settings.check_solution_len,
        0..=(settings.swap_count as usize - 1),
    ));
    ui.end_row();

    ui.label("Check solution retries")
        .on_hover_text("If a shorter solution is found, how many times to retry before giving up.");
