...
impl BackgroundAnimation {
    ...
    fn update_cell(data: &mut BackgroundData, cell: &Cell, dt: f32) {
        ...
        let rotation = if stop {
            Rotation::None
        } else {
            cell.rotation_for_fill()
        };
        ...
        let direction = if rotation == Rotation::CounterClockwise {
            ...
            && data.rotation_t > drotation
            && data.rotation_t < 1.0 - drotation
        {
            data.last_direction
        } else {
            None
    ...
    pub fn draw_background_cell(
        &mut self,
        painter: &Painter,
        palette: &Palette,
        cell: &Cell,
        dt: f32,
        draw_data: BackgroundAnimationDrawData,
    ) {
        let BackgroundAnimationDrawData {
            index,
            center,
            scale,
            show_hint,
        ...
        painter.rect(
            ...
            palette.background,
            Stroke::new(data.swap_glow * scale * 0.02, stroke_color),
        );
        ...
        if data.rotation_glow > 0.001 {
            ...
            let color = palette
                .get(data.last_rotation_color.unwrap_or(Color::CCW))
                .gamma_multiply(0.95 * data.rotation_glow * data.rotation_glow);
            for i in 0..dot_count {
                let theta = dot_angle_range * (i as f32 + data.rotation_t);
                let cos = theta.cos();
                let sin = theta.sin();
                let radius =
                    f32::powf(cos.abs().powi(6) + sin.abs().powi(6), -0.16666666) * scale * 0.35;
        ...
        fn update_hint(
            ...
            if *hint_glow > 0.001 {
                painter.rect(
                    ...
                    Rounding::same(scale * 0.05),
                    palette.get(Color::SWAP).gamma_multiply(*hint_glow * 0.33),
